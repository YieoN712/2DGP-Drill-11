...
class Zombie:
    ...
    def get_bb(self):
        if self.size == 200:
            return self.x - 60, self.y - 100, self.x + 65, self.y + 75
        elif self.size == 150:
            return self.x - 45, self.y - 75, self.x + 55, self.y + 55

    def handle_collision(self, group, other):
        if group == 'zombie:ball' and self.size == 200:
            self.size = 150
            self.y -= 25
        elif group == 'zombie:ball' and self.size == 150:
            game_world.remove_object(self)
        elif group == 'zombie:boy':
            self.frame = 0
            game_over_font = load_font('ENCR10B.TTF', 50)
            clear_canvas()
            game_world.render()
